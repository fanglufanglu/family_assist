...
- 默认连接当前 ECS `http://47.238.240.30`；Android 仅对白名单 IP 临时放行 HTTP。正式上架前必须为账号密码和协助数据配置可信 HTTPS 证书
- 密码在服务端以 PBKDF2-SHA256 带盐哈希保存；当前 ECS 的 HTTP 传输仅用于部署联调，配置可信 HTTPS 是正式上架前的阻断项
...
后台提供运营总览、用户与亲属关系、协助会话、问题诊断和管理审计。“用户”页同时展示账号当前是否连接 Relay、最后活跃时间、设备和 APP 版本；20 秒内收到认证请求或 APP 心跳视为在线，页面每 10 秒自动刷新。连接状态为账号级汇总，不展示 IP，也不提供用户屏幕画面、完整手机号、密码或会话令牌。
